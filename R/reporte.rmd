...
juegos$Year <- as.integer(juegos$Year)
...
reAccion$Year<-seq.int(nrow(reAccion))


reAccion$Year2 = reAccion$Year^2
reAccion$Year3 = reAccion$Year^3
reAccion$Year4 = reAccion$Year^4
poly_reg = lm(formula = totalSales ~ .,
              data = reAccion)

ggplot() +
  geom_point(aes(x = reAccion$Year , y = reAccion$totalSales),
             colour = 'red') +
  geom_line(aes(x = reAccion$Year, y = predict(poly_reg, newdata = reAccion)),
            colour = 'blue') +
  ggtitle('Comportamiento de Ventas de los juegos de género de Acción') +
  xlab('Año') +
  ylab('Ingreso Total')

predict(poly_reg, data.frame(Year = 18,
                             Year2 = 18^2,
                             Year3 = 18^3,
                             Year4 = 18^4))
...
```
```{r}
m1 <- lm(reAccion$totalSales~reAccion$Year)
summary(m1)
plot(reAccion$Year, reAccion$totalSales, xlab = "Año", 
      ylab = "ventas", pch = 16)
abline(lsfit(reAccion$totalSales, reAccion$Year)) 
```
